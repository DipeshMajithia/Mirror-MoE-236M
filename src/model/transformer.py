...
from .experts import MLP  # Needs MLP import

@dataclass
class ModelArgs:
    dim: int = 512  # Small scale for 100M proxy
    n_layers: int = 8
    head_dim: int = 64
    hidden_dim: int = 1408 # Standard intermediate size
    n_heads: int = 8
    vocab_size: int = 32000
    norm_eps: float = 1e-5
    
    # MoE specific
    use_moe: bool = True     # Toggle for Dense Validation
    num_experts: int = 32
    num_experts_per_tok: int = 2
    # Shared expert budget: 15-20% of standard FFN params
    # 15% of 1408 approx 212
...
class TransformerBlock(nn.Module):
    ...
    def __call__(self, x: mx.array, mask: mx.array = None, temperature: float = 1.0):
        ...
        if self.args.use_moe:
            out, gate_probs, topk_indices = self.moe(self.norm2(h))
            return h + out, gate_probs, topk_indices
        else:
...
class MirrorTransformer(nn.Module):
    ...
    def __call__(self, x: mx.array, temperature: float = 1.0):
        ...
        for layer in self.layers:
            h, probs, indices = layer(h, mask, temperature=temperature)
            all_gate_probs.append(probs)
            all_topk_indices.append(indices)
            
        h = self.norm(h)
        out_logits = self.output(h)
        
        return out_logits, all_gate_probs, all_topk_indices
